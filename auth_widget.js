const HW = {
  ...
  async request(path, method='GET', body=null) {
    ...
    try {
      ...
      try {
        ...
        console.error('HW API non-JSON response:', text.substring(0, 300));
        return { error: 'Server error. Please try again.' };
      }
    } catch(e) {
      console.error('HW API fetch error:', e);
  ...
  updateUI() {
    ...
    const wishBtn = document.getElementById('hw-wishlist-btn');
    const isAdmin = this.user && this.user.role === 'admin';
    if (cartBtn) cartBtn.style.display = isAdmin ? 'none' : '';
    if (wishBtn) wishBtn.style.display = isAdmin ? 'none' : '';
  ...
  async loadWishCount() {
    ...
    if (b && data.count > 0) { b.textContent = data.count; b.style.display = 'flex'; }
  },
  ...
  async doLogin() {
    ...
    let data;
    try {
      data = await this.request('auth.php?action=login','POST',{email,password:pass});
    } catch(e) {
  ...
  async doRegister() {
    ...
    const name  = document.getElementById('hw-reg-name').value.trim();
    const email = document.getElementById('hw-reg-email').value.trim();
    const phone = document.getElementById('hw-reg-phone').value.trim();
    const pass  = document.getElementById('hw-reg-pass').value;
    ...
    btn.textContent = 'Create Account →'; btn.disabled = false;
    if (!data.success) { err.textContent = data.error||'Registration failed. Please try again.'; return; }

    document.getElementById('hw-register-form').innerHTML = `
      <div style="text-align:center;padding:20px 0">
        <div style="font-size:56px;margin-bottom:16px">📧</div>
        <h3 style="color:#fff;font-family:'Barlow Condensed',sans-serif;font-size:22px;font-weight:900;text-transform:uppercase;margin-bottom:10px">Check Your Email</h3>
        <p style="color:#666;font-size:14px;line-height:1.7">We've sent a verification link to<br><strong style="color:#E87000">${email}</strong><br><br>Click the link in the email to activate your account. Check spam if you don't see it.</p>
        <button onclick="HW.closeAuth()" style="margin-top:20px;background:#E87000;color:#fff;border:none;padding:11px 28px;border-radius:6px;font-family:'Barlow Condensed',sans-serif;font-size:14px;font-weight:700;letter-spacing:1px;text-transform:uppercase;cursor:pointer">Got it!</button>
      </div>`;
  },
  ...
  init() {
    ...
    const btn = document.getElementById('hw-account-btn');
    if (btn) btn.onclick = () => this.user ? this.showAccountMenu() : this.openAuth();
