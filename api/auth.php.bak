...
switch ($action) {
    ...
    // ============================================================
        ...
        // Add verify columns if they don't exist yet (safe, idempotent)
        $db->query("ALTER TABLE users ADD COLUMN IF NOT EXISTS verify_token VARCHAR(100) NULL");
        $db->query("ALTER TABLE users ADD COLUMN IF NOT EXISTS verify_expires DATETIME NULL");
    ...
    // ============================================================
        ...
        $db->prepare("UPDATE users SET verified=1, verify_token=NULL, verify_expires=NULL WHERE id=?")
    ...
    // ============================================================
        ...
        $db->prepare("UPDATE users SET verify_token=?, verify_expires=? WHERE id=?")
           ->bind_param('ssi', $tok, $exp, $user['id'])->execute();
        $db->close();
    ...
    // ============================================================
        ...
        $db->prepare("UPDATE users SET password=? WHERE id=?")->bind_param('si', $hash, $auth['uid'])->execute();
        $db->close();
    ...
    // ============================================================
        ...
        $db = getDB();
        $db->prepare("UPDATE users SET name=?, phone=? WHERE id=?")->bind_param('ssi', $name, $phone, $auth['uid'])->execute();
        $db->close();
